...
const { uninstallMergeDriver } = require('./install-merge-driver');
...
async function runUninstall(options = {}) {
  ...
  // Strip the merge driver wiring before removing _Sprintpilot/, otherwise
  // the .gitattributes block points at a script that's about to disappear.
  try {
    await uninstallMergeDriver(projectRoot);
  } catch (err) {
    console.warn(
      pc.yellow(`WARN: merge driver cleanup failed (${err.message || err})`),
    );
  }
