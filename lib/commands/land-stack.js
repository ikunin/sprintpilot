...
async function landOne(projectRoot, pr, opts, log) {
  ...
  if (merge.exitCode !== 0) {
    ...
    // files automatically. Check what's left.
    ...
    // No conflicts remain — auto-merge driver covered everything. Stage
    // and commit.
    log('  auto-merge driver resolved doc conflicts — committing merge…');
    const stage = await tryGit(projectRoot, ['add', '-u']);
    if (stage.exitCode !== 0) {
      throw new HaltError(`git add failed after auto-merge: ${stage.stderr.trim()}`);
    }
