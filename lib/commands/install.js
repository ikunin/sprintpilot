...
async function readExistingAutopilotConfig(projectRoot, v1Snapshot) {
  const out = {
    ...
    // Fast-lane master switch (autopilot.fast_lane.enabled). null = not set
    // in user config → inherit the bundled default (false).
    fastLaneEnabled: null,
    // Fast-lane AC-count budget. null = inherit the bundled default (3).
    fastLaneMaxAc: null,
  };
  let raw = null;
  ...
  // `fast_lane:` block → `enabled: true|false`. Scoped to WITHIN the block
  // (header, then any indented lines — comments OR other knobs the user may
  // have reordered above it — then the active `enabled:` line). Without the
  // tolerance, a hand-reordered block would read null and a `--yes` reinstall
  // would silently flip a user's `enabled: true` back to false.
  const fastLaneMatch = raw.match(
    /^[ \t]*fast_lane:[^\n]*\n(?:[ \t]+[^\n]*\n)*?[ \t]+enabled:[ \t]*(true|false)/m,
  );
  if (fastLaneMatch) {
    out.fastLaneEnabled = fastLaneMatch[1] === 'true';
  }
  // `fast_lane.max_ac` — an active (non-commented) line, scoped to WITHIN the
  // fast_lane block (after its header, past any comment/enabled lines) so a
  // stray `max_ac:` elsewhere can't be misread. Null when only the commented
  // default is present.
  const maxAcMatch = raw.match(
    /^[ \t]*fast_lane:[^\n]*\n(?:[ \t]+[^\n]*\n)*?[ \t]+max_ac:[ \t]*(\d+)[ \t]*(?:#.*)?$/m,
  );
  if (maxAcMatch) {
    const n = Number.parseInt(maxAcMatch[1], 10);
    if (Number.isFinite(n) && n >= 0) out.fastLaneMaxAc = n;
  }
  return out;
}
...
// Rewrite the `enabled:` line inside the nested `fast_lane:` block. Kept
// separate from applyScalar because applyScalar targets top-level (2-space)
// scalars under `autopilot:`, whereas fast_lane.enabled is nested one level
// deeper. Three shapes:
//   1. block present with `enabled:` line     → in-place replace
//   2. block present without an `enabled:` line → insert one under the header
//   3. block absent                             → append a fast_lane block
function applyFastLaneEnabled(source, enabled) {
  const literal = enabled ? 'true' : 'false';
  // (1) block header immediately followed (allowing comment lines) by an
  // enabled: line — replace the value in place.
  const enabledRe =
    /^([ \t]*fast_lane:[ \t]*(?:#.*)?\n(?:[ \t]*#.*\n)*[ \t]*enabled:[ \t]*)(?:true|false)([ \t]*(?:#.*)?)$/m;
  if (enabledRe.test(source)) {
    return source.replace(enabledRe, (_m, head, tail) => `${head}${literal}${tail || ''}`);
  }
  // (2) block header present but no enabled: line — insert one right after.
  const headerRe = /^([ \t]*)fast_lane:[ \t]*(?:#.*)?$/m;
  const headerMatch = source.match(headerRe);
  if (headerMatch) {
    const indent = headerMatch[1];
    return source.replace(headerRe, (m) => `${m}\n${indent}  enabled: ${literal}`);
  }
  // (3) no fast_lane block — append one under the autopilot: block.
  if (!/^autopilot:\s*$/m.test(source)) return source;
  const trimmed = source.endsWith('\n') ? source : `${source}\n`;
  return `${trimmed}  fast_lane:\n    enabled: ${literal}\n`;
}

// Write `max_ac` inside the nested `fast_lane:` block. Replaces an existing
// active `max_ac:` line, else inserts one right after the block's `enabled:`
// line (which always exists once the block is written). No-op on a bad value
// or when the block is absent.
function applyFastLaneMaxAc(source, value) {
  const n = Number.parseInt(String(value), 10);
  if (!Number.isFinite(n) || n < 0) return source;
  // Replace an existing active `max_ac:` — scoped to the fast_lane block (like
  // the insert path below) so a stray indented `max_ac:` elsewhere can't be
  // rewritten. The prefix (header + intervening lines) is preserved verbatim.
  const scopedActiveRe =
    /^([ \t]*fast_lane:[^\n]*\n(?:[ \t]+[^\n]*\n)*?)([ \t]+)max_ac:[ \t]*\d+([ \t]*(?:#.*)?)$/m;
  if (scopedActiveRe.test(source)) {
    return source.replace(
      scopedActiveRe,
      (_m, prefix, indent, tail) => `${prefix}${indent}max_ac: ${n}${tail || ''}`,
    );
  }
  // Insert after the fast_lane block's `enabled:` line — scoped to the block
  // (header, any indented lines, then enabled) so a future indented `enabled:`
  // key above fast_lane can't misdirect the insertion.
  const scopedEnabledRe =
    /^([ \t]*fast_lane:[^\n]*\n(?:[ \t]+[^\n]*\n)*?)([ \t]+)enabled:[ \t]*(?:true|false)[^\n]*\n/m;
  if (scopedEnabledRe.test(source)) {
    return source.replace(scopedEnabledRe, (m, _prefix, indent) => `${m}${indent}max_ac: ${n}\n`);
  }
  return source;
}
...
async function patchAutopilotConfig(
  projectRoot,
  { sessionStoryLimit, retrospectiveMode, autoPlanOnStart, fastLaneEnabled, fastLaneMaxAc },
) {
  ...
  // Fast lane — nested keys, own writers.
  if (fastLaneEnabled !== undefined && fastLaneEnabled !== null) {
    updated = applyFastLaneEnabled(updated, fastLaneEnabled === true);
    // Only persist max_ac when the lane is ON (writing it while OFF is noise).
    if (fastLaneEnabled === true && fastLaneMaxAc !== undefined && fastLaneMaxAc !== null) {
      updated = applyFastLaneMaxAc(updated, fastLaneMaxAc);
    }
  }
  if (updated !== original) {
...
async function resolveAutopilotSettings({ projectRoot, yes, dryRun, v1Snapshot }) {
  ...
  // Fast lane — opt-in default false; preserve existing user choice on upgrade.
  const defaultFastLane = existing.fastLaneEnabled ?? false;
  const defaultFastLaneMaxAc = existing.fastLaneMaxAc ?? 3;
  ...
  if (yes) {
    if (
      ...
      existing.fastLaneEnabled != null
    ) {
      console.log(
        pc.dim(
          `Preserving autopilot config: session_story_limit=${defaultLimit}, retrospective_mode=${defaultMode}, auto_plan_on_start=${defaultAutoPlan}, fast_lane=${defaultFastLane}`,
        ),
      );
    }
    return {
      ...
      fastLaneEnabled: defaultFastLane,
      fastLaneMaxAc: defaultFastLaneMaxAc,
    };
  }
  ...
  if (dryRun) {
    console.log(
      pc.dim(
        `[DRY RUN] Would prompt for autopilot config (current: session_story_limit=${defaultLimit}, retrospective_mode=${defaultMode}, auto_plan_on_start=${defaultAutoPlan}, fast_lane=${defaultFastLane})`,
      ),
    );
    return {
      sessionStoryLimit: defaultLimit,
  ...
  // Fast lane — an opt-in relaxation of the RED-first rule for LOW-RISK
  // stories only. Default OFF. When on, a conservative pre-story gate routes
  // trivial stories (docs, small config, tiny pure functions) through
  // bmad-quick-dev one-shot to save tokens/time, while substantial stories
  // keep the full 7-step cycle. Tests are still required and a misclassified
  // story falls back to the full cycle, so the downside is bounded.
  const fastLaneEnabled = await prompts.confirm({
    message:
      'Enable the quick-dev FAST LANE? (Routes only LOW-RISK stories through one-shot quick-dev under full profiles — saves tokens/time. A conservative gate defaults to the full cycle on any doubt, tests stay required, and a failed fast-lane story re-runs the full cycle. Default OFF.)',
    initialValue: defaultFastLane,
  });

  // Only tune the size gate when the lane is actually on. The other knobs
  // (allow/deny globs, require_story_tag) stay config-file edits under
  // autopilot.fast_lane.* — documented in Sprintpilot.md.
  let fastLaneMaxAc = defaultFastLaneMaxAc;
  if (fastLaneEnabled) {
    const maxAcRaw = await prompts.text({
      message:
        'Fast lane: max Acceptance Criteria for a story to qualify (more ACs → always full cycle)',
      initialValue: String(defaultFastLaneMaxAc),
      validate(value) {
        if (value == null || value === '') return undefined;
        const n = Number.parseInt(value, 10);
        if (!Number.isFinite(n) || String(n) !== String(value).trim() || n < 0) {
    ...
    fastLaneMaxAc =
      maxAcRaw == null || String(maxAcRaw).trim() === ''
        ? defaultFastLaneMaxAc
        : Number.parseInt(String(maxAcRaw).trim(), 10);
  }

  return { sessionStoryLimit, retrospectiveMode, autoPlanOnStart, fastLaneEnabled, fastLaneMaxAc };
}
...
async function runInstall(options = {}) {
  ...
  // 1b. Soft compatibility check — advisory only. Warns when the detected BMad
  //     version is below the supported floor or newer than the tested-through
  //     ceiling, but NEVER blocks: presence (step 1) is the only hard gate.
  try {
    const bounds = await readCompatBounds(path.join(ADDON_DIR, 'manifest.yaml'));
    const compat = checkBmadCompat(bmadVersion, bounds);
    if (compat.level !== 'ok') {
      console.log(pc.yellow(`WARNING: ${compat.message}`));
    }
  } catch {
    // An advisory check must never break an install.
  }
  console.log('');
  ...
  const { sessionStoryLimit, retrospectiveMode, autoPlanOnStart, fastLaneEnabled, fastLaneMaxAc } =
    await resolveAutopilotSettings({
      projectRoot,
      yes,
      dryRun,
      v1Snapshot: v1ConfigSnapshot,
  ...
  console.log(
    `    ${apKey('autopilot.fast_lane.enabled')}${apVal(String(fastLaneEnabled))} Route low-risk stories through one-shot quick-dev under full profiles (default off)`,
  );
  console.log('');
