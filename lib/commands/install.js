...
const { installMergeDriver } = require('./install-merge-driver');
...
async function runInstall(options = {}) {
  ...
  // 6d. Wire the auto-merge driver for BMad state YAML files. Idempotent;
  //     skips cleanly when the project isn't a git repo yet (driver
  //     registration is deferred until next install after `git init`).
  try {
    await installMergeDriver(projectRoot, { dryRun });
  } catch (err) {
    console.warn(
      pc.yellow(
        `WARN: merge driver registration failed (${err.message || err}); state-file conflicts will fall back to default 3-way merge.`,
      ),
    );
  }
