...
// Parse the user's existing `_Sprintpilot/modules/autopilot/config.yaml` (if
// any) so interactive prompts can default to the current values. Regex-based
// so we don't add a YAML parser dep for two scalar fields. Unrecognized /
// unreadable files fall back to bundled defaults.
async function readExistingAutopilotConfig(projectRoot) {
  ...
  // `session_story_limit: 3`  — unquoted integer
  const limitMatch = raw.match(/^\s*session_story_limit:\s*(\d+)\s*$/m);
  if (limitMatch) {
  ...
  const modeMatch = raw.match(/^\s*retrospective_mode:\s*["']?([a-zA-Z_-]+)["']?\s*$/m);
...
async function patchAutopilotConfig(projectRoot, { sessionStoryLimit, retrospectiveMode }) {
  ...
  let updated = original.replace(/^(\s*session_story_limit:\s*)\S.*$/m, `$1${sessionStoryLimit}`);
  updated = updated.replace(/^(\s*retrospective_mode:\s*)\S.*$/m, `$1${retrospectiveMode}`);
  if (updated !== original) {
    await writeAtomic(file, updated);
  }
}

async function resolveAutopilotSettings({ projectRoot, yes, dryRun }) {
  const existing = await readExistingAutopilotConfig(projectRoot);
...
async function runInstall(options = {}) {
  ...
  console.log(
    `    autopilot.session_story_limit    ${sessionStoryLimit}      Stories to fully implement per run (0 = unlimited)`,
  );
  console.log(
    `    autopilot.retrospective_mode     ${retrospectiveMode}   Epic-end retrospective: auto (inline) | stop (pause) | skip (not recommended)`,
  );
  console.log('');
