# /sprintpilot-update — Check and Install Sprintpilot Updates

## Workflow

1. **Read current version** from `_Sprintpilot/manifest.yaml` (the `version:` field under `addon:`).
...
3. **Compare versions.**
   - If they match: report "Sprintpilot v{version} is up to date." and stop.
...
4. **Show what's new.** Fetch the changelog summary for the user:
   ...
   To see the full changelog: https://github.com/ikunin/sprintpilot/releases
   ```
