...
| `story_land` (only under `merge_strategy: land_as_you_go`) | The story branch is merged into the base branch AND base is pushed (the land plan's `git merge` + `git push origin <base>` steps). **`git_steps_completed: true` in success output** — set ONLY after every land step exited 0. Canonical signal; when omitted the verifier probes `git merge-base --is-ancestor <commit_sha> origin/<base>` (so echo `commit_sha`) and accepts when it confirms. Squash merges can't be confirmed by sha, so under `squash_on_merge` the explicit flag is required. Skipping the merge/push leaves the story unlanded — the orchestrator will refuse to start the next story (and auto-recovers by rewinding to finish this one). |
