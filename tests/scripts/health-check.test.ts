...
import { createTempRepoWithRemote, gitIn, type TempRepo, writeRaw } from './helpers/repo.js';
...
describe('health-check', () => {
  ...
  it('clean worktree with done status classified as CLEAN_DONE', () => {
    ...
    const r = runScript(
      'health-check',
      ['--worktrees-dir', '.worktrees', '--status-file', 'status.yaml'],
      { cwd: repo.dir },
    );
    expect(r.status).toBe(0);
  ...
  it('non-worktree directory classified as DIRTY (inherits parent git context)', () => {
    ...
    writeFileSync(
      join(repo.dir, '.worktrees', 'orphan-dir', 'file.txt'),
      'not a git repo\n',
      'utf8',
    );
    const r = runScript('health-check', ['--worktrees-dir', '.worktrees'], { cwd: repo.dir });
  ...
  it('custom base branch works', () => {
    ...
    const r = runScript(
      'health-check',
      ['--worktrees-dir', '.worktrees', '--base-branch', 'develop'],
      { cwd: repo.dir },
    );
    expect(r.status).toBe(0);
