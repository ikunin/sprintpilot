...
function makeBmadCore(root: string) {
  ...
  writeFileSync(
    join(root, "_bmad/_config/manifest.yaml"),
    'bmad:\n  version: "6.2.0"\n',
  );
  mkdirSync(join(root, "_bmad/bmm"), { recursive: true });
  writeFileSync(
    join(root, "_bmad/bmm/config.yaml"),
    "project:\n  name: v1-migration-test\n",
  );
}

function seedV1(root: string, opts: { withUserGitConfig?: boolean; withUserTemplates?: boolean } = {}) {
  ...
  writeFileSync(join(v1, "modules/ma/config.yaml"), "multi_agent:\n  enabled: true\n  max_parallel_analysis: 7  # user-edited\n");
  writeFileSync(join(v1, "modules/autopilot/config.yaml"), "autopilot:\n  session_story_limit: 5  # user-edited\n");
...
describe("v1 → v2 migration", () => {
  ...
  it("snapshot preserves user-customized templates, not just config.yaml (P13)", () => {
    ...
    const prBody = readFileSync(
      join(dir, V2_ADDON, "modules/git/templates/pr-body.md"),
      "utf8",
    );
  ...
  it("repeated migration does not clobber the pristine backup (P14)", () => {
    ...
    const timestamped = entries.filter((f) =>
      /^AGENTS\.md\.bak-sprintpilot-migration-/.test(f),
    );
