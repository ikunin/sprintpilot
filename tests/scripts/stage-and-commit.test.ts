...
describe('stage-and-commit', () => {
  ...
  it('secrets allowlist skips matching files', () => {
    ...
    const r = runScript(
      'stage-and-commit',
      ['--message', 'add config', '--allowlist', 'allowlist.txt'],
      { cwd: repo.dir },
    );
    expect(r.status).toBe(0);
  ...
  it('custom max-size-mb is respected', () => {
    ...
    const r = runScript('stage-and-commit', ['--message', 'add medium', '--max-size-mb', '0'], {
      cwd: repo.dir,
    });
    expect(r.status).toBe(0);
  });
  ...
  it('file-list cross-reference warns on unexpected files', () => {
    ...
    const r = runScript('stage-and-commit', ['--message', 'test', '--file-list', 'filelist.md'], {
      cwd: repo.dir,
    });
    expect(r.status).toBe(0);
  ...
  it('files larger than 2 MB skip secret scan', () => {
    ...
    const r = runScript('stage-and-commit', ['--message', 'update big', '--max-size-mb', '100'], {
      cwd: repo.dir,
    });
    expect(r.status).toBe(0);
