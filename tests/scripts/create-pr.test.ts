...
describe('create-pr', () => {
  ...
  it('git_only platform returns SKIPPED and exits 2', () => {
    ...
    const r = runScript(
      'create-pr',
      ['--platform', 'git_only', '--branch', 'story/1-1', '--title', 'Test PR'],
      { cwd: repo.dir },
    );
  ...
  it('missing branch flag exits 1', () => {
    const r = runScript('create-pr', ['--platform', 'github', '--title', 'Test'], {
      cwd: repo.dir,
    });
    expect(r.status).toBe(1);
  });
  ...
  it('missing title flag exits 1', () => {
    const r = runScript('create-pr', ['--platform', 'github', '--branch', 'story/1-1'], {
      cwd: repo.dir,
    });
    expect(r.status).toBe(1);
  });
  ...
  it('no remote configured returns SKIPPED and exits 2', () => {
    const r = runScript(
      'create-pr',
      ['--platform', 'github', '--branch', 'story/1-1', '--title', 'Test PR'],
      { cwd: repo.dir },
    );
    expect(r.status).toBe(2);
    expect(r.stdout.split('\n')[0]).toBe('SKIPPED');
  ...
  it('unknown platform exits 1', () => {
    ...
    const r = runScript(
      'create-pr',
      ['--platform', 'unknown_platform', '--branch', 'story/1-1', '--title', 'Test'],
      { cwd: repo.dir },
    );
    expect(r.status).toBe(1);
