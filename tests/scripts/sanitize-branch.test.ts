...
describe("sanitize-branch", () => {
  ...
  it("long name is truncated with hash", () => {
    const longName = "this-is-a-very-long-story-key-that-exceeds-the-sixty-character-limit-for-branch-names";
  ...
  it("custom max-length is respected", () => {
    const r = runScript("sanitize-branch", ["a-moderately-long-name", "--max-length", "15"], { cwd: repo.dir });
    expect(r.status).toBe(0);
  ...
  it("custom prefix is used for validation", () => {
    const r = runScript("sanitize-branch", ["my-feature", "--prefix", "feature/"], { cwd: repo.dir });
    expect(r.status).toBe(0);
  ...
  it("max-length below 8 is rejected", () => {
    const r = runScript("sanitize-branch", ["long-story-name", "--max-length", "5"], { cwd: repo.dir });
    expect(r.status).toBe(1);
