...
export function assertFileContains(path: string, pattern: RegExp): void {
  ...
  if (!pattern.test(content)) {
    throw new Error(`Expected ${path} to match ${pattern}, got:\n${content.slice(0, 500)}`);
  }
}
 ...
 */
export function assertYamlField(filePath: string, fieldPath: string, expected: unknown): void {
...
export function assertYamlFieldExists(filePath: string, fieldPath: string): void {
  const data = readYaml(filePath);
  const keys = fieldPath.split('.');
  let current: unknown = data;
  ...
  if (current === undefined || current === null) {
    throw new Error(`YAML field '${fieldPath}' is null/undefined in ${filePath}`);
  }
}
...
export function assertCommitMessageMatches(
  ...
  if (!pattern.test(log)) {
    throw new Error(`Expected a commit matching ${pattern} in last ${maxCommits} commits:\n${log}`);
  }
}
...
export function assertCleanWorkingTree(repoDir: string): void {
  ...
  if (status.length > 0) {
    throw new Error(`Expected clean working tree in ${repoDir}, got:\n${status}`);
  }
}
...
export function assertTestsPass(dir: string, command: string): void {
  try {
    ...
    throw new Error(`Tests failed in ${dir}:\n${error.stdout ?? ''}\n${error.stderr ?? ''}`);
  }
}

export function assertMarkdownHasSections(filePath: string, sections: string[]): void {
  ...
  if (missing.length > 0) {
    throw new Error(`${filePath} missing sections: ${missing.join(', ')}`);
  }
}
