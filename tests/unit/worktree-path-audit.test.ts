...
// PR 10 audit: no Sprintpilot script should treat `<worktree>/.git` as a
// directory. In a git worktree the `.git` entry is a file pointing at
// the parent repo's `.git/worktrees/<name>/`, so `ls <worktree>/.git/
// refs/...` and friends break silently. Use `git -C <worktree>
// rev-parse --git-dir` or similar git porcelain instead.
