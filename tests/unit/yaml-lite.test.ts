...
const { yamlSafe, hasStoryBlock, replaceStoryBlock, appendStoryBlock, readStoryField } =
  yamlMod as {
...
`;
  ...
  it('replaceStoryBlock updates in place, preserves neighbours', () => {
    ...
    expect(out).toContain('push_status: pushed');
    // Only one 1-1 block
    const count = (out.match(/^ {2}1-1:/gm) || []).length;
    expect(count).toBe(1);
  });
  ...
  it('replaceStoryBlock is idempotent across many upserts (no blank-line growth)', () => {
    ...
    const h2 = lines.findIndex((l) => /^ {2}1-2:/.test(l));
