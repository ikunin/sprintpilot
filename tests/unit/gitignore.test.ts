...
const {
  ...
  resolveIgnoreFile: (projectRoot: string) => Promise<{ path: string; created: boolean; usedExclude?: boolean }>;
  addIgnoreEntry: (ignoreFile: string, entry: string, opts?: { dryRun?: boolean }) =>
...
describe("gitignore", () => {
  ...
  beforeEach(() => { dir = mkdtempSync(join(tmpdir(), "bmad-gi-")); });
