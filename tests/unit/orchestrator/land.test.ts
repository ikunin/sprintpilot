...
describe('planLand: land_when modes', () => {
  it('no_wait emits snapshot + land — no polling step', () => {
  ...
  it('ci_pass adds a CI-check polling step', () => {
    ...
    const r = planLand(story(), profile, baseOpts());
    expect(r.steps).toHaveLength(3); // snapshot + wait + land
    const wait = r.steps[1];
